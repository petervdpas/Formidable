export function initYamlEditor(containerId, onSaveCallback) {
    ...
    function renderEditor(data) {
      ...
      container.innerHTML = `
        ...
        <fieldset>
          ...
          <div id="fields-list">
            ${currentData.fields.map((f, idx) => renderFieldBlock(f, idx)).join("")}
          </div>
          <div class="form-row">
        ...
        <div class="button-group">
          <button id="save-yaml">Save</button>
        </div>
      `;
    ...
    function renderFieldBlock(field, index) {
      return `
        <div class="field-block" data-idx="${index}">
          ...
          <div class="form-row field-options-row" style="${field.type === "dropdown" ? "" : "display: none;"}">
            <label>Options</label>
            <input type="text" class="field-options" placeholder="Comma-separated" value="${(field.options || []).join(', ')}" />
          </div>
    ...
    function wireEvents() {
      ...
      container.querySelector("#save-yaml").addEventListener("click", () => {
        ...
        container.querySelectorAll("#fields-list .field-block").forEach((block) => {
          ...
          const options = block.querySelector(".field-options")?.value
            .split(",")
            .map((s) => s.trim())
            .filter(Boolean);
