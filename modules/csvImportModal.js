  ...
  for (let i = 0; i < csvHeaders.length; i++) {
    ...
    function updateTransformed() {
      try {
        ...
        const myVal = applyTransformRule(rawVal, { rule: currentRule, param: liveParam }, { mode: "preview" });
        ...
        if (allReady) {
          ...
          for (let j = 0; j < csvHeaders.length; j++) {
            ...
            const oParam = oDef?.type === "number"
              ...
              : (nInputs[oh]?.value || "");
            parts.push(applyTransformRule(oRaw, { rule: oRule, param: oParam }, { mode: "preview" }));
          }
          if (parts.length > 1) {
            isConcat = true;
